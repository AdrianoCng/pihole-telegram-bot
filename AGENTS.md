...
**Bot** (`src/bot.js`): Exports a configured Telegraf instance. Registers authentication, typing, commands, greetings, help, fallback replies, and the central error handler (`bot.catch(handleBotError)`). Importing it does not launch the bot.
...
**Errors** (`src/errors/`):
- `PiholeError` has a `code` (`HTTP`, `INVALID_SESSION`, `INVALID_RESPONSE`), a `status` for HTTP errors, and the static endpoint `path` when known
- `CommandError` (`code` `COMMAND_FAILED`) carries the `exitCode` of a failed sudo command
- `classifyError.js` maps any thrown value to an `ERROR_KINDS` value from `src/constants/errorMessages.js` (`AUTHENTICATION`, `INVALID_RESPONSE`, `NOT_RESPONDING`, `REQUEST_FAILED`, `COMMAND_FAILED`, `UNEXPECTED`)

Error messages must be static strings. User-facing text lives in `src/constants/errorMessages.js` (`DEFAULT_ERROR_MESSAGES`, keyed by kind) and is shared by every command.
...
- `execCommandWithOutput.js`: spawns sudo commands, streams output through a callback, rejects with `CommandError` on a nonzero exit
...
- `logSafeError.js`: logs an allowlist of fields only (`path` defaults to `error.path`, plus `exitCode`); never headers, SIDs, passwords, or payloads
...
**Middleware** (`src/middlewares/`): Directly imported authentication and typing functions, plus `errorHandler.js`. `handleBotError(error, ctx)` is the single error boundary. It classifies the error, logs it safely under `ctx.command`, replies with the default message for its kind, logs any reply failure, and never rethrows.
...
- Errors: Controller/Service throws → `bot.catch(handleBotError)` → `classifyError` → `logSafeError` + `DEFAULT_ERROR_MESSAGES` → User
