...
it("replies to API errors and propagates other errors", () => {
  const log = jest.spyOn(console, "error").mockImplementation(() => {});
  const ctx = createMockContext();
  try {
    registrations.error({ isApiError: true, message: "Unauthorized" }, ctx);
    expect(ctx.reply).toHaveBeenLastCalledWith("Unauthorized", undefined);
    registrations.error({ isApiError: true }, ctx);
